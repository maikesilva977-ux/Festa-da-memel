...
const cardCriancasConfirmadas = document.getElementById(
  "cardCriancasConfirmadas"
);
...
function atualizarDashboard() {
  ...
  // Quantidade de crianças que já confirmaram presença (Vai)
  const totalCriancasConfirmadas = todosOsConvidados.filter(
    (convidado) =>
      convidado.respondeu &&
      convidado.confirmou === true &&
      convidado.menorDe10 === true
  ...
  cardCriancasConfirmadas.textContent = totalCriancasConfirmadas;
}
...
btnLogin.addEventListener("click", async () => {
  ...
  // =========================================================
  // Cria um "tempo limite" de 10 segundos: se o Firebase não
  // responder nesse tempo (rede lenta/instável, bloqueio, etc.),
  // mostramos um erro em vez de ficar preso pra sempre em
  // "Entrando...".
  // =========================================================
  const tempoLimite = new Promise((_, rejeitar) => {
    setTimeout(() => {
  ...
  try {
    // Promise.race "corre" as duas promises e usa a que terminar primeiro
    await Promise.race([
    ...
    if (erro.message === "tempo-esgotado") {
      mensagemErroLogin.textContent =
        "O login demorou demais para responder. Verifique sua conexão (Wi-Fi/dados móveis) e tente novamente.";
    } else {
